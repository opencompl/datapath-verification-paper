...
\begin{itemize} 
  ...
  which enables the algorithm-generic automatic validation of compression trees in dataflow synthesis
...
along with basic operations on the bit heap, such as evaluating a it to a concrete value and adding or removing bits.
...
Lastly, we construct compressor trees and prove that applying one to a bit 
value unchanged (\autoref{subsec:compression}).
...
In all instances the verified datapath pass completed without aborting. None of the chais failed the well-formedness check, and every chain reduced the bit heap to two rows.
...
For our flow, sum the runtime of the two equivalence checks and also include the Lean runtime for the verified datapath engine.
...
arithmetic circuits in VHDL, is built on the bit heap abstraction of~\citet{application_specific_arithmetic} and the compression techniques we formalize in this paper.
