...
\ifx\paperversion\paperversiondraft
  ...
  \makeatletter
  \ifdraftmargins
    % IEEEtran's own two-column measure. \geometry recomputes \textwidth from
    % its defaults, which silently narrows the text block (516pt -> 430pt) and
    % changes line breaking relative to the submission build. Capture the class
    % values first and restore them after, so the draft differs from the
    % submission ONLY by the extra margin-note space, never by column width.
    % IEEEtran's native conference measure. These cannot be read from
    % \textwidth here: loading the geometry package has already overwritten it
    % with geometry's own default (430pt), so the class values are hard-coded.
    \newdimen\ieee@textwidth   \ieee@textwidth=516pt
    \newdimen\ieee@textheight  \ieee@textheight=672pt
    \newdimen\ieee@columnsep   \ieee@columnsep=12pt
    \geometry{asymmetric}
    % Widen the sheet on BOTH sides: 3.5cm of margin note space on the right,
    % and 5cm on the left. Without the matching \oddsidemargin/\evensidemargin
    % shift, the text block stays flush left and left-hand notes (which are
    % typeset to the left of it) fall off the edge of the paper.
    \paperwidth=\dimexpr \paperwidth + 8.5cm\relax
    ...
    % Restore IEEEtran's measure. \geometry finalises the page layout in an
    % \AtBeginDocument hook, so restoring here in the preamble would simply be
    % overwritten; the restore must run in a later hook of our own.
    % \geometry finalises the layout in \AtBeginDocument, and IEEEtran resets
    % the column dimensions there too, so an ordinary \AtBeginDocument hook is
    % overwritten. \AfterEndPreamble runs after both.
    \AfterEndPreamble{%
      \setlength{\textwidth}{\ieee@textwidth}%
      \setlength{\textheight}{\ieee@textheight}%
      \setlength{\columnsep}{\ieee@columnsep}%
