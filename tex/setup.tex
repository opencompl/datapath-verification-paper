...
\ifx\paperversion\paperversiondraft
  ...
  \makeatletter
  \ifdraftmargins
    % The submission measure, i.e. DATE's template block (184mm x 239mm, see
    % the \usepackage[a4paper, total=...]{geometry} call in paper.tex). Widening
    % the sheet below makes \geometry recompute \textwidth from the new
    % \paperwidth, which would change line breaking relative to the submission
    % build. Capture the submission values here and restore them after, so the
    % draft differs from the submission ONLY by the extra margin-note space,
    % never by column width. Hard-coded because \textwidth has already been
    % overwritten by the time this file is read.
    \newdimen\ieee@textwidth   \ieee@textwidth=184mm
    \newdimen\ieee@textheight  \ieee@textheight=239mm
